...
- 10 万行隔离历史库 Unicode 子串查询：最终回归为 50.493 ms。
- Chrome 冷查询：288.168 ms，包含锁定原库检测和 12.3 MB 快照复制。
- Chrome 同会话快照复用查询：8.938 ms。
- Chrome 下拉端到端：379.864 ms。
- 查询期私有工作集：27,557,888 字节（26.28 MiB），低于 30 MiB 目标。
...
- Edge 冷查询：311.186 ms，包含锁定原库检测和 63.8 MB 快照复制。
- Edge 同会话快照复用查询：3.328 ms。
...
| 首次聚焦后再隐藏私有工作集 | 28,971,008 B（27.63 MiB） | 15/25 MiB | **未通过** |
| 搜索完成私有工作集 | 28,721,152 B（27.39 MiB） | 30/50 MiB | 通过 |
| 隐藏空闲 CPU | 0% | 接近 0% | 通过 |
| 温呼出 | 14.968 ms | 100 ms | 通过 |
| 100 次呼出/隐藏 | 28,680,192 B → 29,212,672 B | 增长不超过 1 MiB | 通过 |
| 10 万行查询 | 50.493 ms | 100 ms | 通过 |
| EXE + INI | 490,748 B | 10 MiB | 通过 |
| 正常退出残留进程 | 0 | 0 | 通过 |
...
- Listary 6 的官方命令接口可以传递 `"{query}"`，但没有公开的自定义结果提供器，因此结果仍由本工具下拉框显示。
